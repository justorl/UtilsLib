...
class CustomItem(
    ...
    init {
        if (recipe != null) {
            val key = NamespacedKey(PluginContext.plugin, id)
    ...
    fun attach(item: ItemStack) {
        ...
        itemMeta.persistentDataContainer.set(ItemRegistry.ID, PersistentDataType.STRING, id)
        item.itemMeta = itemMeta
    }

    fun isItem(item: ItemStack): Boolean {
        return item.itemMeta?.persistentDataContainer?.get(ItemRegistry.ID, PersistentDataType.STRING) == id
    }
}